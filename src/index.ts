...
import {
  ...
  type SnowLumaWebSocketClientOptions,
  type WebSocketCloseInfo,
} from '@snowluma/sdk';
import type { OneBotMessageEvent } from '@snowluma/sdk/types';
import type { SnowLumaEventContext } from '@snowluma/sdk/events';
...
function readLogLevel(): LogLevel {
  ...
  if (lvl === 'debug' || lvl === 'warn' || lvl === 'error') return lvl;
  return 'info';
}

/** 构建 SDK 客户端选项。 */
function buildClientOptions(root: RootConfig): SnowLumaWebSocketClientOptions {
  const opts: SnowLumaWebSocketClientOptions = {
...
async function main(): Promise<void> {
  ...
  // 订阅消息事件
  bot.onMessage((event: OneBotMessageEvent, _ctx: SnowLumaEventContext<OneBotMessageEvent>) => {
    // 异步处理，避免阻塞事件分发
    void _ctx;
    handleMessage(bot, event).catch((err: unknown) => {
  ...
  bot.on('close', (info: WebSocketCloseInfo) =>
    logger.warn(`WebSocket 已关闭 (code=${info.code} reason=${info.reason ?? ''})`),
  );
  bot.on('error', (err: unknown) => logger.error('WebSocket 错误:', err));
