...
// ---------------------------------------------------------------------------
/** 根据事件发送回复，自动处理群聊和私聊。 */
async function sendReply(bot, event, msg) {
    try {
 ...
 */
async function sendForwardMsg(bot, target, isGroup, nodes, preview) {
...
function buildForwardNode(illust, isGroup, botId) {
    ...
    return message.node(useAnon ? ANON_FORWARD_QQ : (botId ?? ANON_FORWARD_QQ), useAnon ? ANON_FORWARD_NAME : 'PixivBot', content);
}
/** 创建日榜合并转发节点。 */
function buildDailyRankingNode(illust, index) {
    ...
    return message.node(ANON_FORWARD_QQ, `#${index + 1} ${illust.title}`, [
...
// 帮助 / 状态文案
// ---------------------------------------------------------------------------
function buildHelpLines(prefix) {
    return [
...
async function handleIllustsResult(bot, event, illusts, emptyHint) {
    const state = getState();
    if (illusts.length === 0) {
...
async function handleDailyRanking(bot, event) {
    const state = getState();
