...
export const PLUGIN_CONFIG_FIELDS = [
    {
        key: 'enabled',
        label: '启用插件',
        type: 'boolean',
...
function readConnectionConfig(fromFile) {
    const env = process.env;
    const merged = {
        ...defaultConnectionConfig(),
        ...(fromFile ?? {}),
    };
    if (env.SNOWLUMA_WS_URL)
        merged.wsUrl = env.SNOWLUMA_WS_URL;
    if (env.SNOWLUMA_HTTP_URL)
        merged.httpUrl = env.SNOWLUMA_HTTP_URL;
    if (env.SNOWLUMA_TOKEN)
        merged.accessToken = env.SNOWLUMA_TOKEN;
    if (env.SNOWLUMA_REQUEST_TIMEOUT_MS) {
        const n = Number(env.SNOWLUMA_REQUEST_TIMEOUT_MS);
        if (Number.isFinite(n))
            merged.requestTimeoutMs = n;
    }
...
export function savePluginConfig(cfg, configPath = DEFAULT_CONFIG_PATH) {
    try {
        const dir = dirname(configPath);
        if (!existsSync(dir))
            mkdirSync(dir, { recursive: true });
        const existing = loadConfig(configPath);
        const merged = { ...existing, plugin: cfg };
